...
import { repalceProduct } from "../../redux/api/productSlice";
...
const BuyProduct = () => {
    ...
    const onSubmit = async (data) => {
        setProcessing(true);
        try {
            const prod = products.filter(p => p.id === parseFloat(data.product))
            if (prod[0].quantity > 0) {
                await dispatch(buyProduct({ data, user: user.id }));
                await dispatch(repalceProduct({ productId: data.product }));
                navigate('/customers');
            } else {
