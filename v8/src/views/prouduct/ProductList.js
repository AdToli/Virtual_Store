...
const ProductList = () => {
                            ...
                            {row.purchases.map((pur, index) => (
                                <>
                                    <div key={`${index}`} className="d-flex align-items-center">
                                        ...
                                        <span className="p-2">
                                            <span style={{ cursor: 'pointer', textDecoration: 'underline' }} onClick={() => handleCustomer(pur.customer?.id)}>{pur.customer?.username}</span>
                                        </span>

                                        <span className="p-2">{pur.date}</span>
                                        {(pur.customerID === user.id || user.role === 'admin') && (
    ...
    useEffect(() => {
        if (purchases) {
            ...
            if (user.role === 'admin') {
                ...
                purchases.filter(purc => parseFloat(purc.customerID) === parseFloat(user.id)).forEach(purchase => {
                    const product = products.find(p => p.id === parseFloat(purchase.productID));
    ...
    }, [products, purchases, user.id, user.role, users])
